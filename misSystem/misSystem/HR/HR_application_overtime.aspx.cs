...
{
    public partial class HR_application_overtime : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {                
                ...
                if (Session["check_sn"] != null) //若session存在，則為審核
                {
                    ...
                    str = "select * from hr_application_overtime where SN = " + sn + ";";
                    ...
                    location_text.Text = dt.Rows[0]["location"].ToString().Trim();
                    pay_rad.SelectedValue = dt.Rows[0]["pay"].ToString().Trim();                    
        ...
        protected void send_btn_Click(object sender, EventArgs e)
        {
            ...
            string str = "insert into hr_application_overtime (workerNum, start_time, end_time, content, location, pay, status) values(" + workNum_lab.Text + ", '" + start.ToString(@"yyyy\/MM\/dd H:mm:ss") + "','" + end.ToString(@"yyyy\/MM\/dd H:mm:ss") + "','" + content_text.Text + "','" + location_text.Text + "','" + pay_rad.SelectedValue.ToString() + "'," + status + ");";
            str += " select LAST_INSERT_ID( );";
            Debug.WriteLine(str);
            string sn = GlobalAnnounce.db.GetDataTable(str).Rows[0][0].ToString();
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('The application has been sent.'); window.location.href='HR_list.aspx?SN=" + HR_class.encodeBase64(sn) + "&type=" + HR_class.encodeBase64("1") + "';", true);
        }
        ...
        bool checkInput()
        {
            ...
            if (ff)
            {
                ...
                if ((DateTime.Compare(start, am830) <= 0 && DateTime.Compare(end, am830) <= 0)
                    || (DateTime.Compare(start, pm530) >= 0 && DateTime.Compare(end, pm530) >= 0))
                { datetime_lab.Visible = false; }
                else
                {
                    datetime_lab.Text = "should avoid working time.";
            ...
            if (location_text.Text.Trim() == "")
            {
                location_lab.Text = "Location can't be empty.";
                location_lab.Visible = true;
                flag = false;
            }
            else { location_lab.Visible = false; }
        ...
        protected void edit_btn_Click(object sender, EventArgs e)
        {
            ...
            string str = "update hr_application_overtime set start_time = '" + start.ToString(@"yyyy\/MM\/dd H:mm:ss") + "', end_time = '" + end.ToString(@"yyyy\/MM\/dd H:mm:ss") + "', content = '" + content_text.Text + "', location = '" + location_text.Text + "', pay = '" + pay_rad.SelectedValue.ToString() + "' where SN = " + sn + ";";
            Debug.WriteLine(str);
            GlobalAnnounce.db.InsertDataTable(str);
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('The application has been edited.'); window.location.href='HR_list.aspx?SN=" + HR_class.encodeBase64(sn) + "&type=1';", true);
        }

        protected void delete_btn_Click(object sender, EventArgs e)
        {
            string sn = HR_class.decodeBase64(Request.QueryString["SN"]);
            string str = "update hr_application_overtime set isDelete = 1 where SN = " + sn + ";";
            Debug.WriteLine(str);
            GlobalAnnounce.db.InsertDataTable(str);
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('The application has been deleted.'); window.location.href='HR_list.aspx?SN=-1&type=1';", true);
        }
        ...
        protected void location_text_TextChanged(object sender, EventArgs e)
        {
            if (location_text.Text.Trim() != "") { location_lab.Visible = false; }
        }
        ...
        protected void pass_btn_Click(object sender, EventArgs e)
        {
            ...
            //審核
            string sql = "update HR_application_overtime ";
            switch (status)
            {
